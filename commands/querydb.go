...
func init() {
	queryCommand := cli.Command{
		Name:      "query",
		Usage:     "Queries a specific database, prints results in a CSV",
		ArgsUsage: "<database>",
		Flags: []cli.Flag{
			configFlag,
		},
		Action: func(c *cli.Context) error {
			...
			beaconTable := res.Config.T.Beacon.BeaconTable
			scanTable := res.Config.T.Scanning.ScanTable
			connTable := res.Config.T.Structure.ConnTable
			beacons, scans := res.DB.GetAnomalies(beaconTable, scanTable)

			srcs, dsts := getSrcDstStrings(beacons, scans)

			normConns, anomConns := res.DB.GetSrcDst(srcs, dsts, connTable)
